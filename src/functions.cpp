#include "headers/functions.hpp"
	...
	void ASC::printASCII(BMPPixel& bmppixel)
	{
		std::string result = "";
	...
	std::vector<std::string> ASC::ImageToAscii(BMPPixel& bmppixel)
	{
	...
	void ASC::AsciiToImage(std::vector<std::string>& strings, BMPPixel& bmppixel, std::string path)
	{
		const Cha::Dash dash;
		const Cha::At at;
...
#pragma endregion
	...
	void ASC::ImageToAsciiToImage(BMPPixel& bmppixel, std::string path)
	{
		std::vector<std::string> Ascii = ImageToAscii(bmppixel);
		AsciiToImage(Ascii, bmppixel, path);
	}

	void ASC::ImageToAsciiImage(BMPPixel& bmppixel, std::string path)
	{
	...
	void ASC::FileToAsciiImage(std::string filename, std::string outfilename, int detail_x, int detail_y)
	{
		...
		ASC::ImageToAsciiImage(bmppixel, outfilename);
	}
