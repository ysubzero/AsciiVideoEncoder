...
static void ConsoleFile(const std::vector<std::string>& split, const size_t start, const size_t end, const std::string& directory, const bool color, std::vector<std::string>& results)
{
	...
	for (size_t i = start; i < end; i++)
	{
		if (terminate_program) { break; }
		std::string file = split[i];
		results[i] = func(directory + "\\" + file, 8, 16);
	}
}
...
static int AsciiVideo(const std::string& vidname, const std::string& directory, const int framerate, const bool color, const bool audio)
{
	if (!video(vidname))
	{
	...
	if (!unconverted && !terminate_program)
	{
		...
		std::string create_video;

if (audio)
{
	create_video = "ffmpeg -framerate " + std::to_string(framerate) + " -i \"" + directory + "\\ASCIIoutput_%05d.bmp\" -i \"" + vidname + "\" -loglevel +error -map 0:v:0 -map 1:a:0 -vf \"scale=1920:-2,setsar=1:1\" -c:v libx264 -r 24 -y \"" + directory + "\\asciioutput.mkv\"";
}
else
{
	create_video = "ffmpeg -framerate " + std::to_string(framerate) + " -i \"" + directory + "\\ASCIIoutput_%05d.bmp\" -loglevel +error -map 0:v:0 -vf \"scale=1920:-2,setsar=1:1\" -c:v libx264 -r 24 -y \"" + directory + "\\asciioutput.mkv\"";
}
...
static int Console(const std::string& vidname, const std::string& directory, const int framerate, const bool color, const bool audio)
{
	if (!video(vidname))
	{
		std::print(std::cerr, "Invalid Type\n");
	...
	if (audio)
	{
		command = "ffmpeg -i \"" + vidname + "\" -loglevel +error -vn -acodec pcm_s16le \"" + directory + "\\output.wav\" -vf \"fps = " + std::to_string(framerate) + ",scale=-1:640\" -pix_fmt bgr24 -y \"" + directory + "\\output_%05d.bmp\"";
	}
	else
	{
		command = "ffmpeg -i \"" + vidname + "\" -loglevel +error -vf \"fps = " + std::to_string(framerate) + ",scale=-1:640\" -pix_fmt bgr24 -y \"" + directory + "\\output_%05d.bmp\"";
	}
	std::print("Running ffmpeg...\n");
	...
	else
	{
		...
		const double sleep_t = (1.0 / (static_cast<double>(framerate) * 1.41));
...
static int AsciiImage(const std::string& vidname, const std::string& directory, const int detail, const bool color, const bool audio)
{
	if (video(vidname))
	{
		std::print(std::cerr, "Invalid Type\n");
...
int Text(const std::string& vidname, const std::string& directory, const int detail, const bool color, const bool audio) 	//color remains unused for now.
{
	if (video(vidname))
	{
		std::print(std::cerr,"Invalid Type\n");
...
\tOptional arguments are:\n\t-no_color: do not output color.\n\t-no_audio: do not output audio (recommended for console output.)");
	...
	std::unordered_map<std::string, std::function<int(const std::string&, const std::string&, const int, const bool, const bool)>> functionMap;
	...
	if (argc < 4)
	{
		std::string output = "Not enough arguments.\nCorrect format for command line input is : AsciiVideoEncoder [Command] [Filename] [Framerate/detail] [Optional Arguments].";
	...
	{
		result = functionMap[userinput](vidname, homefolder, frate, color, audio);
	}
	else
	{
